...
void adminGame() {
    ...
    switch (n) {
        ...
        case 1:
            ...
            printf(" Masukan nama game yang ingin dihapus: ");
            getchar();
            gets(find);
            while (fread(&gameSeller, sizeof(gameSeller), 1, ptr) == 1) {
    ...
    switch (o) {
        ...
        case 1:
            ...
            while (fread(&tmp, sizeof(tmp), 1, tp) == 1) {
                while (fread(&refund, sizeof(refund), 1, ptr) == 1) {
                    if (strcmp(refund.game_name, tmp.game_name) != 0 &&
                        refund.stock != tmp.stock) {
...
void buyerLogin(int l) {
    ...
    if (l > 1) {
        ...
        while (fread(&buyer, sizeof(buyer), 1, ptr2) == 1) {
            if (strcmp(username, buyer.username) != 0 &&
                strcmp(password, buyer.password) != 0) {
                printf("\n username dan password salah \n\n");
                system("pause");
                system("cls");
                n = 3;
...
void buyerMenu(char* Mbuyer) {
    ...
    switch (n) {
        ...
        case 3:
            ...
            printf(" Masukan jumlah top up : Rp");
            scanf("%d", &add);
            while (fread(&buyer, sizeof(buyer), 1, ptr) == 1) {
                if (strcmp(buyer.username, Mbuyer) == 0) {
...
void buyerBeli(char* Bbuyer) {
    ...
    scanf("%d", &amount);
    while (fread(&gameSeller, sizeof(gameSeller), 1, ptr) == 1) {
        if (strcmp(game, gameSeller.game_name) == 0 &&
            (gameSeller.stock - amount) >= 0) {
...
void buyerRefund(char* Rbuyer) {
    ...
    scanf("%d", &amount);
    while (fread(&gameBuyer, sizeof(gameBuyer), 1, ptr) == 1) {
        if (strcmp(game, gameBuyer.game_name) == 0 &&
            (gameBuyer.stock - amount) >= 0) {
...
void sellerLogin(int l) {
    ...
    if (l > 1) {
        ...
        while (fread(&seller, sizeof(seller), 1, ptr2) == 1) {
            if (strcmp(username, seller.username) != 0 &&
                strcmp(password, seller.password) != 0) {
                printf("\n username dan password salah \n\n");
                system("pause");
                system("cls");
                n = 3;
...
void sellerMenu(char* Mseller) {
    ...
    switch (n) {
        ...
        case 2:
            ...
            printf(" Banyak game yang akan di upload : ");
            scanf("%d", &o);
            for (i = 0; i < o; i++) {
                getchar();
                printf("\n %d. Nama Game   : ", i + 1);
                gets(gameSeller.game_name);
                printf("    Harga       : ");
                scanf("%d", &gameSeller.game_price);
                printf("    Stok        : ");
                scanf("%d", &gameSeller.stock);
...
void sellerConfirm(char* Cseller) {
    ...
    switch (o) {
        ...
        case 1:
            ...
            while (fread(&tmp, sizeof(tmp), 1, tp) == 1) {
                while (fread(&order, sizeof(order), 1, ptr) == 1) {
                    if (strcmp(order.game_name, tmp.game_name) != 0 &&
                        order.stock != tmp.stock) {
                        fwrite(&order, sizeof(order), 1, ptr2);
                    }
                }
            }
            ...
            while (fread(&tmp, sizeof(tmp), 1, tp) == 1) {
                ...
                ptr = fopen("DGAME-Buyer.dat", "rb");
                while (fread(&buyer, sizeof(buyer), 1, ptr) == 1) {
                    if (strcmp(tmp.buyer, buyer.username) == 0 &&
                        (buyer.balance - tmp.game_price) >= 0) {
                        x = 1;
                    }
                }
                fclose(ptr);
